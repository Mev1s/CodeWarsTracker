...
@app.get("/users", response_model=List[UsersScheme]) # all users bd
def get_users(db: Session = Depends(get_db)) -> List[UsersScheme]:
    users = db.query(UserModel).all()
    return users

@app.get("/user_stats", response_model=List[UserStatsScheme]) # all users_stats bd
def get_user_stats(db: Session = Depends(get_db)) -> List[UserStatsScheme]:
    user_stats = db.query(UserStatsModel).all()
    return user_stats

@app.post("/users", response_model=UsersScheme) # new user to users
...
@app.get("/users/{user_id}", response_model=UsersScheme) # 1 user by his ID
